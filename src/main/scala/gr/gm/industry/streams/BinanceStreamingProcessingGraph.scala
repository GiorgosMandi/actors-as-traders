...
object BinanceStreamingProcessingGraph {
  ...
  /** Builds the end-to-end Akka Stream for live trading on Binance:
    ...
    *   - forwards placed orders to the order-monitoring actor to await
    *     execution reports,
    ...
    * @param net
    *   target Binance network (main/test).
    * @param binanceApiKey
    *   API key for REST + user data stream.
    * @param binanceSecretKey
    *   secret key for REST + user data stream.
    */
  def apply(
      coin: Coin,
      currency: Currency,
    ...
    val priceSource = BinanceStreamSource(tradingSymbol, net)
     val traderFlow: Flow[BookTickerPriceDto, Option[OrderIntent], NotUsed] = ActorFlow.ask(trader)(PriceUpdate.apply)
     val orderExecutionFlow: Flow[OrderIntent, OrderPlacementResponse, NotUsed] = ActorFlow.ask(binanceOrderExecutionActor)(PlaceOrder.apply)
     val orderMonitoringFlow: Flow[PlacedOrder, FinalizedOrder, NotUsed] = ActorFlow.ask(orderMonitorActor)(MonitorOrder.apply)

    priceSource
      .map(ps => {
        system.classicSystem.log.info(s"$ps")
        ps
      })
     .via(traderFlow)
     ...
     .to(Sink.foreach { finalizedOrder =>
       ac.log.info(
         s"Finalized order ${finalizedOrder.orderId} for ${finalizedOrder.symbol}"
       )
     })
  }
}
