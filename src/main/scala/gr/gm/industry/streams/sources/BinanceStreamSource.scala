...
import gr.gm.industry.clients.BinanceEndpointProvider
...
object BinanceStreamSource {
  ...
  def apply(
    ...
    val endpoints = BinanceEndpointProvider.endpoints(net)
    implicit val classicSystem: ActorSystem = system.classicSystem
    val priceWsUri = endpoints.getPriceWsUri(tradingSymbol)
    val webSocketUri = WebSocketRequest(priceWsUri)

    // This queue will push elements from the WebSocket to downstream consumers
    val (queue, source): (
        SourceQueueWithComplete[BookTickerPriceDto],
        Source[BookTickerPriceDto, NotUsed]
    ) =
      Source
        .queue[BookTickerPriceDto](
          bufferSize = 128,
          overflowStrategy = stream.OverflowStrategy.dropHead
        )
        .preMaterialize()
    ...
    val incoming = Sink.foreach[Message] {
      ...
      case TextMessage.Streamed(stream) =>
        stream
          .runFold("")(_ + _)
          .map(text => text.parseJson.convertTo[BookTickerPriceDto])
          .foreach(queue.offer)
      case _ =>
    }
    ...
    val (upgradeResponse, closed) =
      Http().singleWebSocketRequest(webSocketUri, flow)
    ...
    upgradeResponse.onComplete {
      ...
      case Success(upgrade) =>
        classicSystem.log.warning(
          s"WebSocket connection failed: ${upgrade.response.status}"
        )
      case Failure(ex) =>
        classicSystem.log.error(
          s"WebSocket connection failed: ${ex.getMessage}"
        )
    }(ec)
    ...
    closed.onComplete {
      ...
      case Failure(ex) =>
        classicSystem.log.error(s"WebSocket stream failed: ${ex.getMessage}")
    }(ec)
